...
export const createOrder: RouteHandler = async (req, reply) => {
  ...
  // 2. Fetch Items & Calculate Total
  const propertyIds = body.items.map(i => i.property_id);
  const foundProps = propertyIds.length > 0
  ...
  for (const item of body.items) {
    const prop = foundProps.find(p => p.id === item.property_id);
    if (!prop) continue;
  ...
  await db.transaction(async (tx) => {
    await tx.insert(orders).values({
      ...
      currency: itemsToInsert[0].currency || "TRY",
