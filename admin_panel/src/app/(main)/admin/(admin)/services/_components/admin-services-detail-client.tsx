...
export default function AdminServiceDetailClient({ id }: { id: string }) {
  ...
  function onCancel() {
    router.push(`/admin/services?locale=${encodeURIComponent(queryLocale || FALLBACK_LOCALE)}`);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <ServiceForm
        ...
        defaultLocale={queryLocale || apiLocaleFromDb || FALLBACK_LOCALE}
        onSubmit={onSubmit}
        onCancel={onCancel}
        onLocaleChange={(loc: string) =>
          setActiveLocale(localeShortClientOr(loc, queryLocale || FALLBACK_LOCALE))
        }
      />
    </div>
  );
