...
export default function AdminCustomPageDetailClient({ id }: { id: string }) {
  ...
  }, [id, isCreateMode, localesReady, queryLocale, triggerGetById]);
  ...
  const localesForForm: LocaleOption[] = React.useMemo(() => {
    const mapped = (localeOptions ?? []).map((l: any) => ({
      value: String(l.value ?? ''),
      label: String(l.label ?? l.value ?? ''),
    }));
    if (mapped.length > 0) return mapped;

    const fb = localeShortClientOr(queryLocale || apiLocaleFromDb, 'tr');
    return [{ value: fb, label: fb.toUpperCase() }];
  }, [localeOptions, queryLocale, apiLocaleFromDb]);
    ...
    try {
      ...
      if (mode === 'create') {
        const payload: CustomPageCreatePayload = {
          module_key: values.module_key.trim() || 'about',
          locale: loc,
