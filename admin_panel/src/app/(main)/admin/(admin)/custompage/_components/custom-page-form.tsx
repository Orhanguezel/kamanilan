...
const getLocaleFromDto = (dto?: CustomPageDto, fallback = FALLBACK_LOCALE) => {
  const raw = dto?.locale_resolved ?? fallback;
  return norm(raw) || norm(fallback) || FALLBACK_LOCALE;
};
...
const buildInitialValues = (
  ...
  const safeLocale = norm(fallbackLocale || FALLBACK_LOCALE) || FALLBACK_LOCALE;
  ...
  return {
    ...
    module_key: String(initial.module_key ?? '').trim() || 'about',
...
export const CustomPageForm: React.FC<CustomPageFormProps> = ({
  ...
  const t = useAdminT();
  const safeDefaultLocale = norm(defaultLocale || FALLBACK_LOCALE) || FALLBACK_LOCALE;
  ...
  const imageMetadata = useMemo<Record<string, string | number | boolean>>(
    () => ({
      module_key: values.module_key || 'custom_pages',
    ...
    [values.module_key, values.locale, values.slug, values.title, values.page_id, safeDefaultLocale, initialData?.id],
  );
