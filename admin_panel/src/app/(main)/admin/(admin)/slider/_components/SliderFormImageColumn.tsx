...
export type SliderFormImageColumnProps = {
  ...
  disabled?: boolean;
  onChange: (patch: Partial<{ image_url: string; image2_url: string; alt: string }>) => void;
...
export function SliderFormImageColumn({
  ...
  const image2Url = norm(value?.image2_url);
  ...
  return (
    <div className="space-y-3">
      ...
      <AdminImageUploadField
        label="Ikinci Gorsel (Onizleme)"
        helperText="Slider kartinda gosterilecek ikincil gorsel."
        bucket="public"
        folder={folder || 'slider'}
        metadata={effectiveMeta}
        value={image2Url}
        onChange={(url) => onChange({ image2_url: norm(url) })}
        disabled={!!disabled}
        openLibraryHref="/admin/storage"
        onOpenLibraryClick={() => router.push('/admin/storage')}
      />
