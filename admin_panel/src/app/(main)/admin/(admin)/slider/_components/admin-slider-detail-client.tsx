...
export default function AdminSliderDetailClient(props: Props) {
  ...
  const effectiveLocale = React.useMemo(() => {
    ...
    if (set.size > 0) {
      ...
      return localeOptions?.[0]?.value || FALLBACK_LOCALE;
    }
    return q || defaultLocaleFromDb || FALLBACK_LOCALE;
  }, [rawLocaleFromQuery, defaultLocaleFromDb, localeOptions]);

  // ✅ Only skip when we truly cannot query.
  const skipDetail = props.mode !== 'edit' || !id || !effectiveLocale;
  ...
  const loading = props.mode === 'edit' && !skipDetail && (isLoading || isFetching);
