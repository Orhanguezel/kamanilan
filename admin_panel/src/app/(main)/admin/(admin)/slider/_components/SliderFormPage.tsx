...
function buildValuesFromDto(dto: SliderAdminDto | null, locale: string): SliderFormValues {
  ...
  return {
    ...
    image2_url: norm(d.image2_url),
    alt: norm(d.alt),
    badge_text: norm(d.badge_text),
    badge_color: norm(d.badge_color),
    gradient: norm(d.gradient),
...
export function SliderFormPage({
  ...
  const persist = async () => {
    ...
    const payload: any = {
      ...
      image2_url: toNull(values.image2_url),
      alt: toNull(values.alt),
      badge_text: toNull(values.badge_text),
      badge_color: toNull(values.badge_color),
      gradient: toNull(values.gradient),
    ...
    try {
      if (mode === 'create') {
        const created: any = await createSlider(payload as any).unwrap();
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              ...
              <div className="space-y-4">
                <SliderFormImageColumn
                  disabled={disabled}
                  locale={effectiveLoc}
                  value={{ image_url: values.image_url, image2_url: values.image2_url, alt: values.alt }}
                  metadata={{
                    module_key: 'slider',
                    locale: effectiveLoc,
                  ...
                  onChange={(patch) => {
                    ...
                    if (typeof patch.image2_url === 'string')
                      handleChange('image2_url', patch.image2_url);
