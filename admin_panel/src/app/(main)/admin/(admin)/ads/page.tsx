...
export default function AdsPage() {
  ...
  async function createAd(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`${BASE_URL}/admin/ads/banners`, {
      method: "POST",
      credentials: "include",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ ...draft, sourceType: "custom", type: "image", lifecycleStatus: "draft" }),
    });
    if (!response.ok) {
      setError(`Reklam oluşturulamadı (${response.status})`);
      return;
    }
  ...
  async function removeAd(id: number) {
    ...
    if (!response.ok) {
      setError(`Reklam silinemedi (${response.status})`);
      return;
    }
    await load();
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Button asChild variant="outline">
            <Link href="/admin/ads/operations">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Önce taslak oluşturun, ardından detay ekranından yayın ayarlarını tamamlayın.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={createAd} className="grid gap-3 md:grid-cols-2 lg:grid-cols-5">
            <Input
              required
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{items.length} kayıt</CardDescription>
        </CardHeader>
        <CardContent>
          {error ? (
              ...
              {items.map((item) => (
                <TableRow key={item.id}>
                  <TableCell>
                    ...
                    <div className="text-muted-foreground text-xs">{item.advertiser ?? "—"}</div>
                  </TableCell>
                  <TableCell>{item.position}</TableCell>
                  <TableCell>
                    <Badge variant={item.isActive ? "default" : "secondary"}>{item.lifecycleStatus}</Badge>
                  </TableCell>
                  ...
                  <TableCell className="space-x-2 text-right">
                    <Button asChild size="sm" variant="outline">
                      <Link href={`/admin/ads/${item.id}`}>İncele</Link>
                    </Button>
                    <Button size="sm" variant="destructive" onClick={() => void removeAd(item.id)}>
                      Sil
                    </Button>
                  </TableCell>
                </TableRow>
